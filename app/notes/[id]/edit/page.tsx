...
export default async function EditNotePage({ params }: { params: Params }) {
  ...
  return (
    <div className='max-w-3xl mx-auto'>
      <Link href={`/notes/${id}`} className='text-blue-600 hover:underline mb-6 inline-block'>
        &larr; Back to note
      </Link>

      <h1 className='text-2xl font-bold mb-6'>Edit Note</h1>

      <NoteForm mode='edit' noteId={id} initialTitle={note.title} initialContent={initialContent} />
    </div>
  );
}
